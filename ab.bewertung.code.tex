...
\newtoggle{ab@nv@rel}
\toggletrue{ab@nv@rel}
\newtoggle{ab@pkt@taufg}
\newtoggle{ab@pkt@ew}
\newtoggle{ab@komp@anzeigen}
\toggletrue{ab@komp@anzeigen}
...
\pgfkeys{
	/absetup/.cd,
		punkte/erwartungen/.initial = true,
		punkte/erwartungen/.code = \settoggle{ab@pkt@ew}{#1},
		punkte/teilaufgaben/.initial = true,
		punkte/teilaufgaben/.code = \settoggle{ab@pkt@taufg}{#1},
		...
		kompetenzen/anzeigen/.code = \settoggle{ab@komp@anzeigen}{#1},
}
...
%\DeclareExerciseProperty{teilpunkte}
...
\tl_new:N 	\l__ab_cs_aufg_tl
\tl_new:N 	\l__ab_cs_taufg_tl
\tl_new:N 	\l__ab_cs_ew_tl
...
%% Variables
% Kompetenzen
\seq_new:N \g__ab_competencies_seq
...
\cs_new:Npn \ab_taufg_put_ew:nnn #1#2#3 {
	...
	%\seq_gput_right:cx {\l__ab_tmpb_tl} {{#2}{#3}}
	\seq_gput_right:cn {\l__ab_tmpb_tl} {{#2}{#3}}
}
...
\NewDocumentCommand \zusatzpunkte { m m m }{%
	\begin{aufgabe}[ID=#1,subtitle=#2,points=#3,print=false,use=true]
		% Empty
	\end{aufgabe}%
}

\NewDocumentCommand \ordnungspunkte { m }{\zusatzpunkte{OP}{Ordnungspunkte}{#1}}

% Kommandos für Kompetenzen
\NewDocumentEnvironment {kompetenzen} {}
	{}
	{}
\NewDocumentCommand \kompetenz { m }{%
	\seq_gput_right:Nx \g__ab_competencies_seq {#1}%
}
...
\xsimsetup{
	...
	aufgabe/post-hook = {%
		...
		% TODO: Taufg "0" berücksichtigen
		\ab_foreach_taufg:nn {\GetExerciseProperty{ID}} {
			\ab_foreach_erwartung:nn {#1} {
				\seq_put_right:Nx \l__ab_tmpa_seq {
					{#1}
					%{##1}
					{##2}
				}{}
		...
		%
		\SetExpandedExerciseProperty{erwartungen}{%
			\seq_use:Nn \l__ab_tmpa_seq {,}
		}
	}
}
...
\RenewDocumentEnvironment {aufgabenteil} { s o } {\teilaufgaben%
	...
	%\iftoggle{ab@pkt@ew}{\par\flushright \GetExerciseProperty{ID}~\alph{teilaufgabeni}\endflushright}{}%
	\iftoggle{ab@pkt@ew}{\par\Erwartungspunkte}{}%
	\endteilaufgaben
}
...
% #1: Code
% Inside #1 the available parameters are
% 	#1: type of exercise (e.g. "aufgabe")
%		#2: ID
%		#3: number (value of exercise counter)
%		#4: subtitle (as set by subtitle=)
%		#5: points
%		#6: bonus-points
...
% #1: ID of the exercise (as set by xsim) (eg "0")
% #2: Code
%
% Inside #2 the available parameters are
% 	#1: ID of the sub-exercise
%		#2: label
\cs_new:Npn \ab_foreach_taufg:nn #1#2 {
	\cs_set:Nn \__ab_ta_loop_item:nn {#2}
	%
	\tl_set:Nx \l__ab_cs_taufg_tl {\ab__aufg_seq_cs:n {#1}}
	\seq_if_exist:cTF {\l__ab_cs_taufg_tl} {
		\seq_map_inline:cn {\l__ab_cs_taufg_tl} {
...
%% Loop over ewartungen of an exercise
% #1: ID of the (sub-)exercise (eg "aufg1-taufga")
% #2: Code
%
% Inside #2 the available parameters are
% 	#1: descriptive text
...
\cs_new:Npn \ab_foreach_erwartung:nn #1#2 {
	...
	%
	\tl_set:Nx \l__ab_cs_ew_tl {\ab__taufg_ew_seq_cs:n {#1}}
	\seq_if_exist:cTF {\l__ab_cs_ew_tl} {
		\seq_map_inline:cn {\l__ab_cs_ew_tl} {
			\__ab_ew_item_a:xx
				{\tl_item:Nn {##1} {1}}
				{\tl_item:Nn {##1} {2}}
		}
...
% erwartungshorizont/stil = tabellen
\cs_new:Npn \ab__ew_tabellen:n #1 {
	...
	\ab_foreach_taufg:nn {#1} {
		...
		\seq_put_right:Nn \l__ab_tmpa_rows_seq {\ref{taufg:##1} &}
		\seq_put_right:Nx \l__ab_tmpa_rows_seq {\seq_use:Nn \l__ab_tmpa_seq {\newline}}
	...
	\begin{center}\renewcommand{\arraystretch}{1.4}
		...
		\ab_foreach_aufg:n {
			...
			\begin{tabularx}{\textwidth}{|M{1cm}|X|M{1.5cm}|M{1.5cm}|} \hline
				...
				\multicolumn{2}{|l|}{\textbf{Aufgabe~#2}~#4}
					& \textbf{\ab_punkte_aufg:n #2} & \small  \tabularnewline\hline\hline %
				%
...
\DeclareExerciseTableTemplate{tabelle}{\ab@ew@tabelle} % See below

%%% Helper to build the table rows for exercises / sub-exercises
\cs_new:Npn \ab__ew_tabelle: {
	\seq_clear:N \l__ab_tmpa_rows_seq
	\ab_foreach_aufg:n {
		% Header and erwartungen of the exercise
		\ab__ew_tabelle_erwartungen:n {aufg##2-taufg0}
		% ggf. User-ID statt Nummer anzeigen, falls gesetzt
		\seq_put_right:Nn \l__ab_tmpa_rows_seq {\ExercisePropertyIfSetTF{##1}{##2}{ID}{\textbf{\ExercisePropertyGet{##1}{##2}{ID}}}{\textbf{##3}} &}
		% Gibt es Erwartungen der Aufgabe direkt (nicht Taufg)
		\seq_if_empty:NTF \l__ab_tmpa_seq {
			\seq_put_right:Nn \l__ab_tmpa_rows_seq {&}
			% Falls es keine Teilaufgaben gibt, Punkte bei der Aufgabe anzeigen
			\tl_set:Nx \l__ab_cs_taufg_tl {\ab__aufg_seq_cs:n {##2}}
			\seq_if_exist:cTF {\l__ab_cs_taufg_tl} {
				\seq_put_right:Nn \l__ab_tmpa_rows_seq {& \\\hline}
			}{
				\seq_put_right:Nn \l__ab_tmpa_rows_seq {\textbf{##5} & \\\hline}
			}
		} {
			\seq_put_right:Nx \l__ab_tmpa_rows_seq {\seq_use:Nn \l__ab_tmpa_seq {\newline}}
			\seq_put_right:Nn \l__ab_tmpa_rows_seq {& \textbf{##5} & \\\hline}
		}
		% Erwartungen of the sub-exercises
		\ab_foreach_taufg:nn {##2} {
			\ab__ew_tabellen_erwartungen:n {####1}
			\seq_put_right:Nn \l__ab_tmpa_rows_seq {\ref{taufg:####1} &}
			\seq_put_right:Nx \l__ab_tmpa_rows_seq {\seq_use:Nn \l__ab_tmpa_seq {\newline}}
			\seq_put_right:Nn \l__ab_tmpa_rows_seq {& \textbf{\ab_punkte_taufg:n {####1}} & \\\hline}
		}
	}
}
%%% Helper to get a sequence of all erwartungen for an exercise
\cs_new:Npn \ab__ew_tabelle_erwartungen:n #1 {
	\seq_clear:N \l__ab_tmpa_seq
	\ab_foreach_erwartung:nn {#1} {
		\seq_put_right:Nn \l__ab_tmpa_seq {##1}
	}
...
%% erwartungshorizont/stil = schule
\DeclareExerciseTableTemplate{schule}{\ab@ew@schule} % See below

%%% Helper to create the table rows
\cs_new:Npn \ab__ew_schule: {
	\ab_foreach_aufg:n {
		% Header der Aufgabe
		%\seq_put_right:Nx \l__ab_tmpa_rows_seq {\seq_use:Nn \l__ab_tmpa_seq {\newline}}
		%\seq_put_right:Nn \l__ab_tmpa_rows_seq {& \ab_punkte_aufg:n #1 & \\\hline}
		\seq_put_right:Nn \l__ab_tmpa_rows_seq {\rowcolor{muted}}
		\seq_put_right:Nn \l__ab_tmpa_rows_seq {\textbf{Aufgabe~##3}~##4 &}
		\seq_put_right:Nn \l__ab_tmpa_rows_seq {\textbf{##5} & \\\hline}
		% Erwartungen der Aufgabe
		\ab_foreach_erwartung:nn {aufg##2-taufg0} {
			\seq_put_right:Nn \l__ab_tmpa_rows_seq {####1 &}
			\seq_put_right:Nn \l__ab_tmpa_rows_seq {####2 & \\\hline}
		}
		% Erwartungen der Teilaufgaben
		\ab_foreach_taufg:nn {##2} {
			\ab_foreach_erwartung:nn {####1} {
				\seq_put_right:Nn \l__ab_tmpa_rows_seq {\ref{taufg:####1}~########1 &}
				\seq_put_right:Nn \l__ab_tmpa_rows_seq {########2 & \\\hline}
			}
		}
	}
...
\def\ab@ew@schule{
	% Build rows for longtable environment
	\ab__ew_schule:
	%
	\underline{Name: \Large\hspace{6cm}}

	% Create longtable
	\rowcolors{2}{ab.tabelle.zeile.ungerade}{ab.tabelle.zeile.gerade}
	\begin{longtable}{|p{.65\linewidth}|c|c|}\hline
			% Kopfzeile
			\rowcolor{ab.tabelle.kopf.hg}
			Die~Schülerin/der~Schüler\dots & mögl. Punkte & erreicht \tabularnewline\hline\hline %
			%
			%% Aufgaben, Teilaufgaben und Erwartungen
			\seq_use:Nn \l__ab_tmpa_rows_seq {}
			%
			% Fusszeile
			\hline
			\rowcolor{ab.tabelle.kopf.hg}\textbf{Summe:} & \textbf{\ab_punkte_total:} & \tabularnewline\hline
			\iftoggle{ab@nv@rel}{\rowcolor{ab.tabelle.kopf.hg}\textbf{Prozentual:} & \textbf{100\,\%} & \tabularnewline\hline}{}
			\rowcolor{ab.tabelle.kopf.hg}\textbf{Note:} & \multicolumn{2}{c|}{} \tabularnewline\hline
	\end{longtable}
...
\NewDocumentCommand \TeilaufgabenPunkte {O{+}} {%
	\seq_clear:N \l__ab_tmpa_seq
	\ab_foreach_taufg:nn {\GetExerciseProperty{ID}} {
		\seq_put_right:NV \l__ab_tmpa_seq {\ab_punkte_taufg:n {##1}}
	}
	\seq_use:Nn \l__ab_tmpa_seq {~#1~}
}
...
\NewDocumentCommand \PunkteErwartungen {m m O{+}} {%
	\tl_set:Nx \l__ab_tmpa_tl {\ab_taufg_id:nn {#1}{#2}}
	\seq_clear:N \l__ab_tmpa_seq
	\seq_set_split:Nnx \l__ab_tmpb_seq {,} {\ExercisePropertyGet{aufgabe}{#1}{erwartungen}}
	\seq_map_inline:Nn \l__ab_tmpb_seq {
		\tl_set:Nx \l__ab_tmpb_tl {\tl_item:Nn {##1} {1}}
		\tl_if_eq:NNTF \l__ab_tmpa_tl \l__ab_tmpb_tl {
			\seq_put_right:Nx \l__ab_tmpa_seq {\tl_item:Nn {##1} {2}}
		}{}
	}
	\seq_use:Nn \l__ab_tmpa_seq {~#3~}
}
\NewDocumentCommand \ErwartungenPunkte {O{+}} {%
	\tl_set:Nx \l__ab_tmpa_tl {\ab_taufg_id:nn {\GetExerciseProperty{ID}} {\alph{teilaufgabeni}}}
	\seq_clear:N \l__ab_tmpa_seq
	\seq_set_split:Nnx \l__ab_tmpb_seq {,} {\GetExerciseProperty{erwartungen}}
	\seq_map_inline:Nn \l__ab_tmpb_seq {
		\tl_set:Nx \l__ab_tmpb_tl {\tl_item:Nn {##1} {1}}
		\tl_if_eq:NNTF \l__ab_tmpa_tl \l__ab_tmpb_tl {
			\seq_put_right:Nx \l__ab_tmpa_seq {\tl_item:Nn {##1} {2}}
...
\def\Erwartungshorizont{%
	\clearpage
	\ifmodulgeladen{varianten}{\let\Nummer\ab@vari@nummer}{}
	\chead{}\ohead{}\cfoot{}\ofoot{}\ifoot{}
	%\thispagestyle{empty}
	...
	%
	\iftoggle{ab@komp@anzeigen}{
	\seq_if_empty:NTF {\g__ab_competencies_seq} {} {
...
\begin{center}
	...
	\begin{tabularx}{\textwidth}{|m{9cm}|X>{\raggedleft\arraybackslash}m{1.5cm}|} \hline
		...
		\seq_use:Nn {\g__ab_competencies_seq} {& & \\ \hline} & & \\ \hline
	\end{tabularx}
\end{center}
}

\def\Erwartungspunkte{\begingroup%
	\flushright (\ErwartungenPunkte)\endflushright
\endgroup}
