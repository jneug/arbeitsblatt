...
\DeclareExerciseTableTemplate{schule}{\ab@ew@schule}
\def\ab@ew@schule{
	% ForEachUsedExerciseByID breaks for tables. Store all used IDs in
	% a (temporary) seq first and loop over that.
	\seq_clear:N \l_tmpa_seq
	...
	\begin{longtable}{|p{.65\linewidth}|c|c|}\hline
			...
			%
			%% Aufgaben, Teilaufgaben und Erwartungens
			\seq_map_function:NN \l_tmpa_seq \ab@ew@schule@aufgabe
			%&&&\\\hline % Somewhere the table ending is messed up. We need to add a blank row for some reason.
			%
			% Fusszeile
			\hline
...
\newcommand{\ab@ew@schule@aufgabe}[1]{%\rowcolor{muted}
	\rowcolor{muted}\textbf{Aufgabe~#1} &
	\textbf{\PunkteAufgabe{#1}} &
	%
	\tl_set:Nx \l__ab_tmpa_tl {\ab_taufg_id:nn {#1} {0}}
	...
	\seq_if_exist:cTF {\l__ab_cs_taufg_tl} {
		\seq_map_inline:cn {\l__ab_cs_taufg_tl} {
			\tl_set:Nx \l__ab_tmpa_tl {\tl_item:Nn {##1} {1}}
			\ab_foreach_erwartung:nn {\l__ab_tmpa_tl} {
				\tabularnewline \hline
				\ref{taufg:\tl_item:Nn {##1} {1}}~
				####1 & ####2 &
			}
		}
	}{}
